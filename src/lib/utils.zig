...
pub fn generateNavigationTree(io: std.Io, alloc: Allocator, root_dir: std.Io.Dir, root_file: []const u8, document_root: *const Block) !NavBarItem {
    ...
    // Track visited paths to prevent infinite cycles.
    var visited: std.ArrayList([]const u8) = .empty;
    defer visited.deinit(alloc);
    try visited.append(alloc, root_link.uri);

    root_link.children = try generateNavBarRecurse(io, alloc, &visited, root_dir, document_root);

    return root_link;
}

fn generateNavBarRecurse(io: std.Io, alloc: Allocator, visited: *std.ArrayList([]const u8), current_dir: std.Io.Dir, block: *const Block) !std.ArrayList(NavBarItem) {
    ...
    for (links) |link| {
        ...
        if (link.kind == .local and std.mem.eql(u8, std.fs.path.extension(link.uri), ".md")) {
            ...
            // Check if this document has already been visited (cycle detection).
            var already_visited: bool = false;
            for (visited.items) |existing_uri| {
                if (std.mem.eql(u8, nav_item.uri, existing_uri)) {
                    already_visited = true;
                    break;
                }
            }

            // Always read the document to get its title (if available).
            // Only recurse if its path has not already been visited.
            if (readFile(io, arena.allocator(), dir, nav_item.uri)) |contents| {
                ...
                if (!already_visited) {
                    // Push onto path stack; shrinkRetainingCapacity pops it after recursion.
                    const path_len = visited.items.len;
                    try visited.append(alloc, nav_item.uri);
                    defer visited.shrinkRetainingCapacity(path_len);
                    ...
                    nav_item.children = try generateNavBarRecurse(io, alloc, visited, child_dir, &child_doc);
                }
            } else |_| {}
